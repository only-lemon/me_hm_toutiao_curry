...
<script>
import store from '@/store/SessionStorage'
...
export default {
  ...
  methods: {
    ...
    myUpload (result) {
      ...
      this.$cuicui.patch('user/photo', formdata).then(res => {
        // 上传成功  res 响应对象
        this.$message.success('修改头像成功')
        // 预览
        this.userForm.photo = res.data.data.photo
        // 更新本地存储
        store.setUserToken({ photo: this.userForm.photo })
        // 更新home的头像
    ...
    async getUserInfo () {
      ...
      const { data: { data } } = await this.$cuicui.get('user/profile')
      this.userForm = data
    },
    async saveUserInfo () {
      // patch 做修改的时候使用的请求方式  局部修改
      // put 做修改的时候使用的请求方式  完整修改
      await this.$cuicui.patch('user/profile', {
        // 严格按照后台的要求传参
      ...
      // 更新本地存储
      store.setUserToken({ name: this.userForm.name })
      // 更新HOME组件的用户名
